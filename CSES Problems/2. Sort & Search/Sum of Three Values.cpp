...
ll cmp(pll a,pll b) {return a.F<b.F;}
...
ll BinarySearch(ll L,ll R,ll val)
{
    while(L<=R)
    {
        ...
        else if (a[M].F>val) return R=M-1;
        else L=M+1;
    }
    return -1;
}
...
int main()
{
    ...
    FOR(i,1,n-2)
    {
        ...
        while(j<=k)
        {
            ll temp=a[i].F+a[j].F+a[k].F;
            if (temp==sum && a[i].S!=a[j].S && a[j].S!=a[k].S && a[k].S!=a[i].S) 
                return cout << a[i].S << " " << a[j].S << " " << a[k].S,0;
            else if (temp>sum) --k;
            else ++j;
        }
    }
